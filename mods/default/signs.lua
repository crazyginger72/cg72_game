...
signs_lib.wall_sign_text_pos = {
    {delta = {x =  0,     y = 0.15, z =  0.850}, yaw = 0},
    {delta = {x =  0.850, y = 0.15, z =  0    }, yaw = math.pi / -2},
    {delta = {x =  0,     y = 0.15, z = -0.850}, yaw = math.pi},
    {delta = {x = -0.850, y = 0.15, z =  0    }, yaw = math.pi / 2},
}
...
signs_lib.update_sign = function(pos, fields)
	...
	local sign_info
	if minetest.get_node(pos).name == "default:sign_yard" then
		sign_info = signs_lib.yard_sign_text_pos[minetest.get_node(pos).param2 + 1]
	elseif minetest.get_node(pos).name == "default:sign_hanging" then
	...
	else --if minetest.get_node(pos).name == "default:sign_post" then
